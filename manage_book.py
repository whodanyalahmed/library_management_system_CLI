...
def search():
    ...
    if choice=='1':
        ...
        bkname = None
        while bkname==None:
            bkname = vd.entry('Title','string','book',int(50))
        ldm.Search(bkname,ttype,0)
    elif choice == '4':
        bkauth = None
        while bkauth==None:
            bkauth = vd.entry('Author','string','book',int(20))
        ldm.Search(bkauth,ttype,0)
    elif choice == '5':
        bkpubn = None
        while bkpubn==None:
            bkpubn = vd.entry('Publication','string','book',int(30))
        ldm.Search(bkpubn,ttype,0)
...
def display():
    ldm.Display(ttype)
def Sort_Titles():
    titles = ldm.get_Titles()
    sorted_list = ldm.bubble_sort_title(titles)
    return sorted_list
def update():
    search = None
    while search==None:
