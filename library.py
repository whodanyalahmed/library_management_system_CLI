...
class Library_Management_System:
    ...
    class Book:
        ...
        def Show_duedt(self):
            ...
            print('{0:9} | {1:30} | {2:10} | {3:13} | {4:7} | {5:50} | {6}\n'.format(
                'RES ID', 'FULLNAME', 'CONTACT', 'ISBN NO', 'USER', 'BOOK TITLE', 'DUE ON'))
            with open('bin\\books\\reserved_book.txt') as res_b:
                for i in res_b:
                    ...
                    if delta >= 0:
                        ...
                        print('{0:9} | {1:30} | {2:10} | {3:13} | {4:7} | {5:50} | {6}'.format(
                            i[0], i[1].replace('_', ' '), i[2], i[3], i[4], i[5].replace('_', ' '), delta))
        ...
        def Reservation_status_print(self):
            ...
            with open('bin\\books\\reserved_book.txt') as lines:
                print('{0:9} | {1:30} | {2:10} | {3:13} | {4:7} | {5:50} | {6:10} | {7:10} |\n'.format(
                    'RES ID', 'FULLNAME', 'CONTACT', 'ISBN NO', 'USER', 'BOOK TITLE', 'ISSUED ON', 'EXPIRY'))
                for line in lines:
                    line = line.strip().split('-')
                    print('{0:9} | {1:30} | {2:10} | {3:13} | {4:7} | {5:50} | {6:10} | {7:10} |'.format(line[0], line[1].replace(
                        '_', ' '), line[2], line[3], line[4], line[5].replace('_', ' '), line[6].replace(',', '/'), line[7].replace(',', '/')))
        ...
        def Return_Process(self, day, con, reserve):
            ...
            with open('bin\\books\\book-info.txt') as search:
                ...
                find = b_li[0]+'-'+b_li[1]+'-'+b_li[2]+'-'+b_li[3] + \
                    '-'+b_li[4]+'-'+b_li[5]+'-'+b_li[6]+'-'+b_li[7]
                convert = b_li[0]+'-'+b_li[1]+'-'+b_li[2]+'-'+b_li[3]+'-' + \
                    b_li[4]+'-'+b_li[5]+'-'+str(int(b_li[6])+1)+'-'+b_li[7]
            ...
            with open('bin\\books\\returned_book.txt', 'a')as f:
                f.write('%s-%s-%s-%s\n' % (str(con),
                        reserve[3], str(day), datetime.now().strftime('%Y,%m,%d/%H:%M:%S')))
            print('-------------------------------')
        ...
        def Book_request(self, uid, bid, utype):
            ...
            if self.Reservation_status():
                print(
                    'Already reserved a book, please return the book before requesting new book.')
                li = self.reserve
                print('{0:10} : {1}\n{2:10} : {3}\n{4:10} : {5}\n{6:10} : {7}\n'.format('Username', li[1].replace(
                    '_', ' '), 'Book title', li[5].replace('_', ' '), 'ISBN', li[3], 'Issued on', li[6].replace(',', '/')))
            else:
                flag = 0
                with open('bin\\books\\book-info.txt') as f:
                    for line in f:
    ...
    class Library_database:
        ...
        def Update(self, find, convert, ttype):
            ...
            if ttype == 'user':
                ...
                if flag == 1:
                    ...
                    print('Book found : ')
                    print(*conf)
                    find = conf[0]+'-'+conf[1]+'-'+conf[2]+'-'+conf[3] + \
            ...
            if ttype == 'user':
                with open('bin\\users\\user-info.txt') as f:
                    print('{0:9} | {1:31} | {2:30} | {3:10} | {4}\n'.format(
                        'USER ID', 'FULLNAME', 'EMAIL ID', 'CONTACT', 'USER TYPE'))
                    for line in f:
                        line = line.strip().split('-')
                        print('{0:9} | {1:31} | {2:30} | {3:10} | {4}'.format(
                            line[0], line[1].replace('_', ' '), line[2], line[3], line[4]))
            else:
                with open('bin\\books\\book-info.txt') as f:
                    print('{5:9} | {0:50} | {1:20} | {2:30} | {3:13} | {4}\n'.format(
                        'BOOK TITLE', 'AUTHOR', 'PUBLISHER', 'ISBN NO.', 'QUANTITY/LEFT', 'BOOK ID'))
                    for line in f:
                        line = line.strip().split('-')
                        print('{0:9} | {1:50} | {2:20} | {3:30} | {4:13} | {5:2} / {6:2}'.format(line[0], line[1].replace(
                            '_', ' '), line[2].replace('_', ' '), line[3].replace('_', ' '), line[4], line[5], line[6]))
                ...
                if send == 0:
                    if flag == 1:
                        ...
                        print('{0:31} | {1:30} | {2:10} | {3}\n'.format(
                            'FULLNAME', 'EMAIL ID', 'CONTACT', 'USER TYPE'))
                        print('{0:31} | {1:30} | {2:10} | {3}'.format(
                            found[1].replace('_', ' '), found[2], found[3], found[4]))
                    else:
                        print('-------------------------------')
                ...
                if send == 0:
                    if flag == 1:
                        ...
                        print('{0:50} | {1:20} | {2:30} | {3:13} | {4}\n'.format(
                            'BOOK TITLE', 'AUTHOR', 'PUBLISHER', 'ISBN NO.', 'QUANTITY/LEFT'))
                        print('{0:50} | {1:20} | {2:30} | {3:13} | {4:2}/{5:2}'.format(found[1].replace(
                            '_', ' '), found[2].replace('_', ' '), found[3].replace('_', ' '), found[4], found[6], found[5]))
                    else:
                        print('-------------------------------')
        ...
        def bubble_sort_title(self,arr):
            n = len(arr)
            for i in range(n):
